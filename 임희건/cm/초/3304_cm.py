# 가영이가 사용하는 텍스트 에디터에는 "찾아 바꾸기"라는 기능이 있습니다.
# 찾고 싶은 내용과 바꾸고 싶은 내용이 입력되면 커서 위치와 같거나 그 뒤에 있는 위치에서 해당 내용을 찾아서 바꾸는 기능입니다. 
# 조금 더 구체적으로 설명하면, 현재 텍스트 에디터에 입력된 전체 내용을 S,
# 현재 커서 위치를 cur (0 ≤ cur ≤ |S|),
# 찾고 싶은 내용을 s,
# 바꾸고 싶은 내용을 t라고 했을 때,
# S[i .. i+|s|-1] = s를 만족하는 최소의 i (i ≥ cur)를 찾아서 t로 바꾸면 됩니다. 
# 만약 해당하는 i가 존재하지 않는 경우, 아무 동작도 하지 않습니다.
# 처음에 텍스트 에디터에 입력되어 있던 내용과 찾아 바꾸기를 사용한 이력이 입력될 때, 텍스트 에디터에 입력되어 있던 내용이 어떻게 변해있을 지 알아내는 프로그램을 작성해주세요.
