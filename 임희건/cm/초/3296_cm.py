# 구간 단속이란, 차량이 도로의 특정 구간을 통과하는데 걸린 시간과 구간의 길이를 이용해 속도위반여부를 판정하는 방법입니다. 구간을 통과하는데 걸린 시간은 (차량이 구간 도착점의 카메라에 촬영된 시각 - 차량이 구간 출발점의 카메라에 촬영된 시각) 입니다.
...
# 모든 로그의 촬영일은 같으며, 같은 차량에 대해 출발점에서 촬영된 시각은 항상 도착점에서 촬영된 시각보다 앞섭니다. 한 카메라에 같은 차량번호가 두번 이상 주어지지 않습니다. 출발점의 카메라에 등장하는 차량은 반드시 도착점의 카메라에도 등장합니다.
