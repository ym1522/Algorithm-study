...
# 하지만 이런 충돌 판정은 이미지의 윤곽이 복잡할수록 훨씬 큰 비용이 듭니다. 만약 실시간 온라인 격투 게임에서 이런 판정 방식을 사용하면 어떻게 될까요? 복잡한 충돌 판정으로 인해 서버에 비용이 쌓이고, 사용자에게 '딜레이'가 발생하는 불상사가 발생하게 됩니다.
# 비용이 큰 위 방법 대신, 충돌 마스크를 사용할 수 있습니다. 충돌 마스크란 화면에 출력되는 이미지 대신 충돌 판정을 수행하는 보이지 않는 다각형입니다.
# 여러분은 가장 간단한 다각형인 정사각형을 충돌 마스크로 사용해보려 합니다. 각각 서로 다른 두 개체의 충돌 마스크에 해당하는 두 정사각형의 정보가 주어집니다. 두 정사각형이 모두 네 변이 x축 또는 y축과 평행할 때, 두 정사각형이 서로 접하거나 겹쳐 있는지 판정해주세요.
